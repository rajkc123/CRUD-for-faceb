...
def submit():
    conn = sqlite3.connect('CRUD.db')
    c = conn.cursor()
    c.execute("INSERT INTO CRUD VALUES (:f_name, :l_name, :age,:address,:city,:zipcode,:password,:gender)", {
        'f_name': f_name.get(),
        'l_name': l_name.get(),
        'age':age.get(),
        'address':address.get(),
...
def query():
    conn = sqlite3.connect('CRUD.db')
    c = conn.cursor()
    c.execute("SELECT *, oid FROM CRUD")
    records = c.fetchall()
    print(records)
    conn.commit()
    conn.close()
...
def delete():
    conn = sqlite3.connect('CRUD.db')
    c = conn.cursor()
    c.execute("DELETE from CRUD WHERE oid = " + delete_box.get())
...
def edit():
    ...
    c = conn.cursor()
    record_id = delete_box.get()
    c.execute("SELECT * FROM CRUD WHERE oid = " + record_id)
    records = c.fetchall()
